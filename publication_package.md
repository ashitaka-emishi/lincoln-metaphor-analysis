...
4. Read [Annotation Codebook](docs/methodology/annotation-codebook.md) for MIPVU, CMT, Koenigsberg, absence, confidence, and ambiguity rules.
5. Use [Controlled Outputs](analysis/controlled_outputs.md) before relying on any aggregate count.
6. Use [Claim-To-Source Audit](synthesis/claim_audit.md) to trace major claims back to instance IDs, sentence IDs, document metadata, and source URLs.
7. End with [Findings](synthesis/findings.md) and [Final Conclusions](synthesis/final_conclusions.md).
...
The `pipeline` command validates existing JSON, rebuilds concordance and analysis, regenerates Stage 4A evidence chains, Stage 4B reliability artifacts, Stage 6A controlled outputs, and the Stage 8 claim audit.
