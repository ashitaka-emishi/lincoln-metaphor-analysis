...
**Evidence from Lincoln corpus** (populate after annotation):
- Instance count of `projected_entity: national_body`: —
- Instance count of `projected_entity: founding_proposition`: —
- Instance count of `projected_entity: covenant_bond`: —

**Structural similarity**: Both treat the projected entity as prior to and more important than individual human lives. Both generate violence as *defense of the object*, not as rational policy choice.

### 2. Obligatory Frame

Both Lincoln and Hitler make violence feel mandatory rather than chosen.

**Lincoln mechanism**: wound (must heal), oath (must enforce), ancestral debt (must pay), Providence (must accomplish God's will)
**Hitler mechanism**: racial survival (must purify), historical destiny (must achieve), existential threat (must eliminate)

**Evidence from Lincoln corpus**:
- Overall obligatory_frame_rate: —
- Rate in formal_public_address: —
- Rate in fragment_private: —

**Structural similarity**: The obligatory frame is arguably the single most important shared element. Both rhetors deny agency — they are not choosing violence; they are fulfilling obligations imposed by prior commitments.
...
**Evidence from Lincoln corpus**:
- Overall sacrificial_economy_rate: —
- Dominant sacrificial_yield: —
- cluster_04 sacrificial_economy_rate: — (expected highest)

**Structural similarity**: Both convert mass death into national investment. Both use manic defense (Kleinian): triumph over loss through the claim that death produces.
...
**Lincoln**: The founders (Washington, Jefferson, Madison) and the Declaration/Constitution
**Hitler**: Germanic/Aryan ancestors and the racial heritage
...
**Analytical significance**: This construct imports epistemic humility into the violence logic. An experiment can fail. A proposition can be disproved. This acknowledgment of fallibility is structurally absent from Hitler's certainty.

**Evidence**: — (populate from concordance)
...
**Analytical significance**: Distributed guilt is the structural precondition for "malice toward none." If both sides are guilty, neither requires permanent punishment.

**Evidence**: — (populate guilt_distribution arc from concordance)
...
**ABSENT FROM LINCOLN**. This is the project's central finding.
...
**Evidence from Lincoln corpus**: Track every candidate instance of disease/purification language. Expected: zero confirmed instances of `disease_and_purification` fantasy type.
...
Hitler's guilt distribution never moves from `external`. The enemies of the Aryan people — Jews, Marxists, Versailles — bear all responsibility. This forecloses reconciliation: if guilt is always external, the guilty must always be punished.
...
| Obligatory frame | Present (wound, oath, debt, Providence) | Present (racial survival, destiny) | Shared mechanism; different justification |
...
| Violence exit condition | Yes (wounds heal, propositions proven, debts paid) | No (pathogen never fully expelled) | **Primary structural divergence** |
...
**Primary structural divergence** (pre-stated; confirm against corpus):

Lincoln's absence of disease_and_purification logic is not a moral accident — it is a structural feature of his metaphor system. The wound heals; the proposition is proven; the oath is fulfilled; the debt is paid; God's punishment runs its course. Every fantasy type in Lincoln's corpus has a logical terminus.
...
**The deeper question**: Why did Lincoln's rhetoric lack this construct? Possible hypotheses (require evidence):
1. Protestant/Calvinist theological tradition emphasized sin and redemption rather than purity
2. Lincoln's legal training privileged contractual and evidentiary logic
3. The founding texts (Declaration, Constitution) provided civic rather than ethnic frameworks
4. Lincoln's personal temperament and the democratic accountability he faced constrained absolutism

This question points beyond the current project toward deeper historical and psychological inquiry.
