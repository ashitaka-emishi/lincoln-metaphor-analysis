...
Human-queue generation writes `stage4m-adjudication-queue.json`, `stage4m-adjudication-queue.csv`, and `stage4m-adjudication-template.csv` under `data/reliability/model-adjudication/`, plus `docs/methodology/stage4m-adjudication-guide.md`. Queue rows preserve the disagreement ID, deterministic queue order, reference and model values, model confidence and rationale context, canonical sentence text, audit and claim links, priority reasons, and one suggested human review question. Generated queue records are immutable inputs: a completed template may retain Stage 4A, document a migration candidate, defer for insufficient evidence, or request codebook clarification, but it may not directly edit a validated annotation stage.

Consensus synthesis writes `model-consensus-report.json` and `model-consensus-report.md` under `data/reliability/model-comparison/`. The JSON preserves its three source paths, an explicit non-decisive interpretation policy, summary counts, stable/unstable/insufficient-evidence field records, ranked document/cluster/category risks, all-model reference challenges, model-supported reference cases, human-review priorities, and codebook implications. `reference_only` items never count as evidence that models support Stage 4A. Cluster risk is limited to explicit cluster-assignment disagreements because the three declared source artifacts do not attach cluster context to every other disagreement.
